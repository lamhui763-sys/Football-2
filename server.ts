...
3. **AI Agent 3 (統計質疑與風險提示官 - 核心反對派)**:
   ...
   - 提供統計學上的理據（例如：期望進球值 xG 回歸均值、主力體能瓶頸、主裁判判罰特點、天氣及氣溫影響、歐冠/國內盃賽雙線作戰的體能分散、市場盤口動態等）。
   - 明確指出高風險警示。

4. **AI Agent 4 (戰術分析師)**:
   - 專門解讀兩隊布陣，深入剖析「球隊陣型對抗」（例如：4-3-3 對陣 4-2-3-1）、「高位逼搶效果」以及「關鍵定位球威脅與空投轟炸風險」。
   - 提供硬核戰術板推演與針對性反制戰術解析。

5. **反駁與整合階段 (Rebuttal & Integration)**:
   - 針對 Agent 3 提出的一系列質疑，Agent 1 和 Agent 2 必須給予理性回應（承認合理的風險，但重申數據支撐的優勢，或者相應微調其原有的預測與信心度）。
   - Agent 2 計算並給出一個調整後的預測比分和更新後的信心度 %。

6. **綜合最終建議 (Final Synthesis)**:
   - 整體觀點完美整合後的結論。
   - 給出最推薦的投注/策略方向（例如：主客勝、大小球、雙重機會、防守型膽碼）。
...
app.post("/api/predict", async (req, res) => {
  try {
    const { message, historicalData } = req.body;
...
app.post("/api/simulate", async (req, res) => {
  try {
    const { homeTeam, awayTeam, focusTopic } = req.body;
